  ...
  export const slice = createSlice({
    ...
    extraReducers: (builder: ActionReducerMapBuilder<GameModel>) => {
      builder
        ...
        .addCase<SnapshotAction>(GameActions.tick, (state, { snapshot }) => {
          if (state.player) {
            ...
            if (collision) {
              ...
              const sound = gameScore(state, state.player.type, lines!);
              if (old_level !== state.level && state.level % 5 === 0) {
            ...
            if (!gameTickRows(state.buffer)) {
              ...
              const player = gamePlayerCreate(next!, state.buffer.width);
              const trans = gameTransform(player, translateNoop, rotateNoop);
              if (gameCollision(trans, state.buffer)) {
                snapshot?.app.sound && gameSound(state, SOUND_FINISHED);
        ...
        .addCase<SnapshotAction>(
          ...
          (state, { snapshot }) => {
            const isRemoved = state.buffer.rows.find((row) => row.removed);
            if (isRemoved) {
        ...
        .addCase(GameActions.bufferPatch, (state, { payload }) => {
          payload.forEach((patch) => gameBufferPatch(state.buffer, patch));
        })
          ...
          (state, { payload, snapshot }) => {
            const ghost =
              snapshot?.app.ghost_piece && state.player
                ? gameGhost(state.buffer, state.player)
                : undefined;
            const screen = gameScreenRender(state.buffer, state.player, ghost);
            return {
              ...state,
              ...payload,
              ghost,
